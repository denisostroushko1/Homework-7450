...
Now we fit an exponential survival model for each group. Results of model are given below. It appears that the two 
groups have different parameters, which are statistically significantly different. 

```{r, echo = F, eval = F}

obj <- survfit(Surv(weeks, relapse) ~1 ,data = l_df[l_df$group == "6-MP", ])
obj2 <- survfit(Surv(weeks, relapse) ~1 ,data = l_df[l_df$group != "6-MP", ])

l_1 <- l_df[l_df$group == "6-MP", ]$weeks %>% unique() %>% sort() %>% unique() %>% length()
l_2 <- l_df[l_df$group != "6-MP", ]$weeks %>% unique() %>% sort() %>% unique() %>% length()

checker_data <- 
  data.frame(
    time = c(l_df[l_df$group == "6-MP", ]$weeks %>% unique() %>% sort(),
             l_df[l_df$group != "6-MP", ]$weeks %>% unique() %>% sort()), 
    
    log_cum_haz = c(log(obj$cumhaz),
                    log(obj2$cumhaz)),
    
    group = c(rep("6-MP", l_1), 
              rep("Placebo", l_2))
  )

ggplot(data= checker_data, 
       aes(x = time, 
           y = log_cum_haz, 
           color = group)) + 
  geom_line() + 
...
for each group using estimate and standard errors from the model output above. 

In order to obtain a confidence interval for each estimate, we fit two separate models for two groups: 

```{r}
summary(survreg(Surv(weeks, relapse) ~ 1 ,data = l_df %>% filter(group == "6-MP"),  dist = "exponential"))
summary(survreg(Surv(weeks, relapse) ~ 1 ,data = l_df %>% filter(group != "6-MP"),  dist = "exponential"))
```

The upper and lower bounds for the 95% confidence interval of the 6-MP group are given by 3.686 $\pm$ 1.96 * 0.333. 
Thus, parameter $\lambda$ for the 6-MP group is 3.686, bounded by (`r round(3.686 - 1.96 * 0.333,3)`, `r round(3.686 + 1.96 * 0.333,3)`). 

Parameter $\lambda$ for placebo group is given by 3.686 - 1.527 = `r 3.686 - 1.527`. 

The upper and lower bounds for the 95% confidence interval of the placebo group are given by `r 3.686 - 1.527` $\pm$ 1.96 * 0.398 
Thus, parameter $\lambda$ for the 6-MP group is `r 3.686 - 1.527`, bounded by (`r round(3.686 -  1.527 - 1.96 * 0.218,3)`, `r round(3.686 -  1.527 + 1.96 * 0.218,3)`). 

We can check our work by calling the `confint` function in R: 
```{r}

confint(survreg(Surv(weeks, relapse) ~ 1 ,data = l_df %>% filter(group == "6-MP"),  dist = "exponential"))
confint(survreg(Surv(weeks, relapse) ~ 1 ,data = l_df %>% filter(group != "6-MP"),  dist = "exponential"))

```

If we look at the confint of the original model we fit, the confidence interval for placebo is given for the difference in 
rates, and not for the $\lambda$ of placebo group. However, it will be useful in the next section. 
...
* P-value: <0.0001. We have enough statistical evidence to conclude that the rates for two groups are different. Therefore
those in the experimental arm 6-MP are more likely to survive for an extended period of time. 
...
# combined variance: since estimates and estimators are approximately normally distributed due to MLE, we can add two variances for two idependednt normal random variables 
...
* Variance for the difference is the sum of varainces of two estimtes, since they are independent normal variables. 
